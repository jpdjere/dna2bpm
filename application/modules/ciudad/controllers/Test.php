...
class test extends MX_Controller {
    ...
    function Index() {
        echo "<h2>Pongo ciudad de juan-borda a Limon</h2>";
        $idciudad = 3520936162;
        $idu_limon = 1574513092;
        $update = array("owner" => $idu_limon);
        $criteria = array("id" => $idciudad);
        $this->db
                ->where($criteria)
                ->update('container.ciudads', $update);
    ...
    function undo() {
        echo "<h2>Pongo ciudad de juan-borda a juan-borda</h2>";
        $idciudad = 3520936162;
        $idu_jb = 1;
...
//        exit;
        ...
        $rs = $this->db->get('container.proyectos_ciudad')->result();

        foreach ($rs as $proj) {
            //$user=$this->user->get_user($proj->idu);
            $query = array('data.Proyectos_ciudad.query.id' => $proj->id);
            $this->db->where($query);
            $this->db->select('id');
    ...
    function check_case() {
        $SQL = "
            SELECT idsent.id AS id, TF3.valor AS ciudad
FROM `td_ciudad` AS TF1
INNER JOIN `td_ciudad` AS TF2 ON TF1.id = TF2.id
INNER JOIN `td_ciudad` AS TF3 ON TF1.id = TF3.id
...
";
        ...
        foreach ($rs->result() as $row) {
            echo $row->id . '<br>';
            $case = array();
            $this->db->where(array('data.Proyectos_ciudad.query.id' => (int) $row->id));
            $this->db->select('id');
//            $this->db->debug=true;
            $case = $this->db->get('case')->result();
            if (count($case)) {
    ...
    function fix_evaluador($case = null) {
        ...
        foreach ($cases as $case) {
            if ($case['id']) {
                ...
                $proy = $this->mongowrapper->db->selectcollection('container.proyectos_ciudad')->findOne(array('id' => $case['id']), array('8668'));
                $ideval = $proy['8668'][0];
