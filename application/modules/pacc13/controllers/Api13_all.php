 ...
 * @date    Jan 28, 2015
 */
class Api13_all extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->postTitle=' H';
        $this->queryTotalPitchs=array('8926'=>array('$exists'=>true),'status'=>'activa');
        $this->queryTotal=array('5689'=>array('$gte'=>20),'status'=>'activa');
        /* LOAD MODEL */
     ...
     */
       ...
       $total = $this->db->where($this->queryTotal)->count_all_results($this->container);
        $query= array(
            "distinct" => "container.orden_de_pago_pacc", 
            "key" => "parent.id",
            "query" =>array(7052=>array('$exists'=>true),'status'=>'activa')
            );
    ...
    function pitchs_evaluados($filter = null, $mode = 'json') {
        ...
        $total = $this->db->where($this->queryTotalPitchs)->count_all_results($this->container);
        $query['8926'] = array('$in'=>array(20,40,50));
        $query['status'] = 'activa';
        $cant = $this->db->where($query)->count_all_results($this->container);
        $label = "$cant";
        $porc = @ intval(($cant / $total) * 100);
    ...
    function Finalizados($filter = null, $mode = 'json') {
        $title = 'Finalizados';
        $total = $this->db->where($this->queryTotal)->count_all_results($this->container);
        $query['5689'] = array('$in'=>array(480,500));
        $query['status'] = 'activa';
        $cant = $this->db->where($query)->count_all_results($this->container);
        $label = "$cant";
        $porc = @ intval(($cant / $total) * 100);
   ...
   /**
    ...
    function cant_proyectos_reclam_cd_venc_crono() {
        //        
    }
