...
class Emprendedor extends MX_Controller {
    ...
    /**
     * Dashboard para emprendedors
     */
    function dashboard($debug=false){
        Modules::run('dashboard/dashboard', 'pacc/json/dashboard_emprendedor.json',$debug);
    ...
    function widget_2doMe2($chunk = 1, $pagesize = 5) {
        ...
        if (isset($data['mytasks'])) {
            foreach ($data['mytasks'] as $k => $mytask) {
                ...
                if (isset($mycase['data']['emprendedors']['query']['id'])) {
                    $emprendedorID = $mycase['data']['emprendedors']['query']['id'];
                    $emprendedor = $this->bpm->get_data('container.emprendedors', array('id' => $emprendedorID));
                    $data['mytasks'][$k]['extra_data']['emprendedor'] = $emprendedor[0]['1693'];
                }
