...
$('#dp_metas').datepicker().on('changeDate',function(ev){
    ...
    var now=moment();
    var periodo = moment(ev.date);
    var dif=moment(now).diff(periodo,'months');
    if(dif>0)
    location.href=globals.module_url+'index/'+dif;
    
    
    // // Pyme
    // $.post(globals.module_url+'get_resumen_visitas',{mes:periodo},function(data){
   	//  $('#wrapper_visitas').html(data);
    // });
    // // Institucional
    // $.post(globals.module_url+'get_resumen_visitas_instituciones',{mes:periodo},function(data){
   	//  $('#wrapper_visitas_instituciones').html(data);
    // });
