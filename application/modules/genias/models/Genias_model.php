...
class Genias_model extends CI_Model {
    ...
    function remove_task($id) {
        ...
        $rs = $this->mongowrapper->db->$container->remove($query);
        return $rs['err'];
    }
    ...
    function add_task($task) {
        ...
        $rs = $this->mongowrapper->db->$container->update($query, $task, $options);
        return $rs['err'];
    }
    ...
    function get_tasks($idu, $proyecto, $periodo = null) {
        ...
        $result = $this->mongowrapper->db->$container->find($query)->sort(array('dia' => -1));
...
//        exit();
    ...
    function add_goal($goal) {
        ...
        $query = array('_id' => $id);
        $mygenia = $this->mongowrapper->db->$container->findOne($query);
        $goal['genia_nombre'] = $mygenia['nombre'];
        $options = array('upsert' => true, 'safe' => true);
        $container = 'container.genias_goals';
        $query = array('id' => (integer) $goal['id']);
        return $this->mongowrapper->db->$container->update($query, $goal, $options);
    }
    ...
    function update_goal() {
        ...
        return $this->mongowrapper->db->$container->update($query, array('$set' => $data), $options);
    }

    function get_goals($idu,$start=null,$end=null) {
        $container = 'container.genias_goals';
        ...
        if(!is_null($start) && !is_null($end))
         $query['desde']=array('$gte'=>$start,'$lte'=>$end);
         
        $goals = $this->mongowrapper->db->$container->find($query)->sort(array('desde' => -1));
        $result = array();
    ...
    function get_case($case) {
        $query = array('id' => $case);
        $container = 'case';
        $result = $this->mongowrapper->db->$container->findOne($query);
        return $result;
    }
    ...
    function config_set($data) {
        ...
        $rs = $this->mongowrapper->db->$container->update($query, $data, $options);
        return $rs['err'];
    }
    ...
    function visitas_remove($container, $id = null) {
        ...
        $visita = $this->mongowrapper->db->$container->remove($query);
        ...
        if ($visita) {
            ...
            $result = $this->mongowrapper->db->$newContainer->findOne($newQuery);
            ...
            if (!empty($resultArr)) {
                ...
                $rs = $this->mongowrapper->db->$newContainer->update($idQuery, $update, $options);
    ...
    function goal_clear_cumplidas($proyecto = '2') {
        ...
        $result = $this->mongowrapper->db->$container_metas->update($query, $update, $options);
        return $result;
    }
    ...
    function goal_update_all($proyecto = '2', $id_visita = null, $idu = null, $fecha = null, $i) {
        ...
        $metas = $this->mongowrapper->db->$container_metas->find($query);
        ...
        foreach ($metas as $meta) {
            ...
            if ($case['status'] == 'closed') {
                ...
                $result = $this->mongowrapper->db->$container_metas->save($meta);
                break;
            }
        }
    ...
    function remove_goal($id) {
        ...
        $my_goal = $this->mongowrapper->db->$container_metas->findOne($query);
        $my_case = $my_goal['case'];

        $result = $this->mongowrapper->db->$container_metas->remove($query);
        ...
        if (!isset($result['err'])) {
            ...
            $rs_case = $this->mongowrapper->db->$container_case->update($query, $data);
        } else {
            return $result['err'];
        }
    }
    ...
    function touch($cuit = null) {
        ...
        $this->mongowrapper->db->$container->update($query, $update);
    }
    ...
    function get_resumen_visitas($periodo) {
        ...
        $container = 'container.genias_visitas';
        $cuits = $this->mongowrapper->db->$container->distinct('cuit');
        // Todos los idus cargados en visitas
        $idus = $this->mongowrapper->db->$container->distinct('idu');

        // Listado de empresas
        $container = 'container.empresas';
        $fields = array('1695', '4651', '1693', '1703');
        $query = array("1695" => array('$in' => $cuits), "4651" => array('$in' => $provincias));
        $mongo_empresas = $this->mongowrapper->db->$container->find($query, $fields);
        ...
        $mongo_usuarios = $this->mongowrapper->db->$container->find($query, $fields);
        $usuarios = array();
        ...
        $visitas = $this->mongowrapper->db->$container->find($query);
...
//var_dump(iterator_to_array($visitas));
    ...
    function get_resumen_visitas_instituciones($periodo) {
        ...
        // Todos los cuits cargados en visitas
        $container = 'container.genias_visitas';
        $cuits = $this->mongowrapper->db->$container->distinct('cuit',array('proyecto'=>4));
        // Todos los idus cargados en visitas
        $idus = $this->mongowrapper->db->$container->distinct('idu',array('proyecto'=>4));
        ...
        $mongo_agencias = $this->mongowrapper->db->$container->find($query, $fields);
...
//exit();
    ...
    // ======= ESTADISTICAS ======= //
      ...
      function estadisticas($proyecto="2",$periodo=null) {

        $stats=array();
        $total_visitas=0;
        $total_metas=0;
        $container = 'container.genias';
        $misgenias = iterator_to_array($this->mongowrapper->db->$container->find()->limit(500));
        ...
        foreach ($misgenias as $genia) {
            ...
            $count_visitas=$this->mongowrapper->db->$container->find(array('idu'=>array('$in'=>$genia['users'])))->count();
            ...
            // Metas
            $container = 'container.genias_goals';
            $query['genia']=(string)$genia['_id'];
            $query['proyecto']=$proyecto;
            // checko si viene periodo
            if(!is_null($periodo))
               $query['desde']=array('$gte'=>$periodo);

            $metas=$this->mongowrapper->db->$container->find($query);
            $count_metas=0;
            $count_cumplidas=0;
            $visitas=array();
         //   var_dump(iterator_to_array($metas));
            ...
            foreach($metas as $meta){
                ...
                //$visitas+=(empty($meta['cumplidas']))?(array()):($meta['cumplidas']);
                
                $visitas=array_merge($visitas,$meta['cumplidas']);
            }
            $stats['genias'][(string)$genia['_id']]['cantidad']=$count_metas;
            $stats['genias'][(string)$genia['_id']]['cumplidas']=$count_cumplidas;
            $stats['genias'][(string)$genia['_id']]['data_visitas']=$visitas;
...
//            echo $genia['nombre']."/".$total_metas."<br>";
     ...
     //=== resumen completo visitas a empresas
     function resumen_empresas($start=null,$end=null) {
        $stats=array();
        $total_visitas=0;
        $total_metas=0;
        $container = 'container.genias';
        $misgenias = iterator_to_array($this->mongowrapper->db->$container->find());
        ...
        foreach ($misgenias as $genia) {
            ...
            $data=array();
            $fields=array('cuit','fecha','nota','idu');

            $query=array('idu'=>array('$in'=>$genia['users']));
            $data['visitas']=iterator_to_array($this->mongowrapper->db->$container->find($query,$fields));
                        
                        
            $stat[$genia['nombre']]=$data;
                    var_dump($genia);
        }
