...
class Fondyf extends MX_Controller {
    ...
    function tile_aprobados_condicional() {
        $this->user->authorize();
        $this->load->model('fondyf/fondyf_model');
        $data ['number'] = count($this->fondyf_model->get_cases_byFilter_container('fondyfpde', 195, array('8334' => '87')));
        $data ['title'] = 'Aprobados Condicional';
        $data ['icon'] = 'ion-document-text';
        $data ['more_info_text'] = 'Listar';
        $data ['more_info_class'] = 'load_tiles_after';
        $data ['more_info_link'] = $this->base_url . 'fondyf/listar_aprobados_condicional';
    ...
    function listar_pp($action = null) {
        ...
        $template = 'fondyf/listar_proyectos_fechas_pp';
        $filter = array(
            'idwf' => 'fondyfpp',
//            'id' => 'VNSU'
        );
...
//        var_dump(json_encode($filter),count($tokens));
        ...
        foreach ($cases as $case) {
            ...
            if (!isset($case_data ['Proyectos_fondyf'] ['8339']))
                continue;
            /* STATUS */
            $status = "N/A";
            if (isset($case_data ['Proyectos_fondyf'] ['8334'])) {
                $status = $option[$case_data ['Proyectos_fondyf'] ['8334'][0]];
            }
            $i++;
            $arr = array(
                'case' => $case['id'],
                ...
                'Nro' => (isset($case_data ['Proyectos_fondyf'] ['8339'])) ? $case_data ['Proyectos_fondyf'] ['8339'] : 'N/A',
            );


//            $arr['fechapresentacion']=isset($case_data['Proyectos_fondyf']['8340'])?date('d/m/Y', strtotime($case_data['Proyectos_fondyf']['8340'])):'???';
            //---saco fecha presentación
            $date = explode('/', $case_data['Proyectos_fondyf']['8340']);
...
//        exit;
    ...
    function listar_aprobados_condicional($action = null) {
        ...
        $this->load->model('fondyf/fondyf_model');
        $option = $this->app->get_ops(772);
        $template = 'fondyf/listar_proyectos_fechas_pde';
        $template = 'fondyf/listar_proyectos';
        $filter = array(
            'idwf' => 'fondyfpde',
        );
        $cases = $this->fondyf_model->get_cases_byFilter_container('fondyfpde', 195, array('8334' => '87'));
//        var_dump($cases[0]);exit;
        ...
        foreach ($cases as $case) {
            ...
            $url = (isset($case_data ['Proyectos_fondyf']['id'])) ? $this->bpm->gateway('../dna2/frontcustom/284/list_docs_fondyf_eval.php?id=' . $case_data ['Proyectos_fondyf'] ['id']) : '#';
            $arr['link_msg'] = '';
            $arr['link_open'] = $url;

            $arr['url_bpm'] = $this->module_url . 'fondyf/escalar_condicional/fondyfpde/' . $case['id'];
            $arr['url_clone'] = '';
            $arr['url_cancelar_pp'] = '';
...
//            $arr['fechapresentacion']=isset($case_data['Proyectos_fondyf']['8340'])?date('d/m/Y', strtotime($case_data['Proyectos_fondyf']['8340'])):'???';
            ...
            $f0 = $this->bpm->get_token('fondyfpp', $case['id'], 'oryx_4EFED47A-7AAD-4728-9577-49F8901AC5B9');
            $arr['fechapresentacionPP'] = (count($f0)) ? date('d/m/Y', strtotime($f0['checkdate'])) : '';
            //---saco fecha presentación
            $f1 = $this->bpm->get_token('fondyfpde', $case['id'], 'oryx_A58D5ECD-6899-4F60-856C-CFE89B36FB91');
            $arr['fechapresentacion'] = (count($f1)) ? date('d/m/Y', strtotime($f1['checkdate'])) : '';
            //-----tomo el evaluador
            $iduser = $case_data ['Proyectos_fondyf'] ['8668'][0];
...
//        var_dump(json_encode($filter),count($tokens));
        $data ['empresas'] = array_map(function ($token) {
            // var_dump($token['_id']);
            $case = $this->bpm->get_case($token ['case'], 'fondyfpp');
            $fondyfpde = $this->bpm->get_case($token ['case'], 'fondyfpde');
            $data = $this->bpm->load_case_data($case);


            $url = (isset($data ['Proyectos_fondyf']['id'])) ? '../dna2/frontcustom/284/list_docs_fondyf_eval.php?id=' . $data ['Proyectos_fondyf'] ['id'] : '#';
            $url_msg = (isset($token ['case'])) ? $this->base_url . 'fondyf/show_msgs/' . $token ['case'] : null;
            /* FonDyF/COORDINADOR (134) */
            $hist=$this->bpm->get_token_history('fondyfpp',$token['case']);
            ...
            $url_clone = (
                    ...
                    and in_array($data ['Proyectos_fondyf'] ['8334'][0], array(60, 65, 68, 87)) //---checkeo que esté en alguno de esos estados
                    ) ? $this->base_url . 'fondyf/clone_case/fondyfpp/fondyfpde/' . $token ['case'] : null;
            //---link para cancelar solo para coordinador
            $url_cancelar_pp = ((in_array(134, $this->id_group) or $this->user->isAdmin()) and $case['status'] == 'open') ? $this->base_url . 'fondyf/cancelar_pp/' . $token ['case'] : null;
            $url_cancelar_pde = (
                    ...
                    ) ? $this->base_url . 'fondyf/cancelar_pde/' . $token ['case'] : null;
            //---link para reevaluar solo para coordinador
            $url_reevaluar_pp = ((in_array(134, $this->id_group) or $this->user->isAdmin()) and $case['status'] == 'open') ? $this->base_url . 'fondyf/reevaluar_pp/' . $token ['case'] : null;
            $url_reevaluar_pde = (
                    (in_array(134, $this->id_group) or $this->user->isAdmin()) and in_array('oryx_3346C091-4A4D-4DCD-8DEC-B23C5FE7F80C', $keys) //---está finalizado pero por esta figura 
                    and $case['status'] == 'closed'
                    ) ? $this->base_url . 'fondyf/reevaluar_pde/' . $token ['case'] : null;
            //---url para checkear

            $url_bpm = '';
            if (in_array(134, $this->id_group) or in_array(135, $this->id_group) or $this->user->isAdmin()) {
                $model = ($fondyfpde) ? 'fondyfpde' : 'fondyfpp';
...
//        var_dump(json_encode($filter),count($tokens));
    ...
    function eliminar_en_preparacion($process = false) {
        ...
        foreach ($tokens as $token) {
            ...
            if ($process)
                $this->bpm->delete_case('fondyfpp', $token['case']);
        }
    }
    ...
    function proyects_amount($filtroproy = null) {
        ...
        $cases = $this->fondyf_model->get_cases_byFilter_container('fondyfpp', 195, $query);
        $cases_arr = array();
        foreach ($cases as $case) {
            $id = $case['data']['Proyectos_fondyf']['query']['id'];
            if (isset($id)) {
                $query = array('id' => $id);
                $values = $this->fondyf_model->get_amount_stats_by_id($query);
...
//         redirect($url);
        if ($id) {
            ...
            echo <<<BLOCK
                <p align='left'>1. <a href="{$dna2url}frontcustom/284/nota_fondyf_preapro.php?id=$id" target="_blank">Nota de Presentac&oacute;n</a></p>
                <p align='left'>2. <a href="{$dna2url}frontcustom/284/print_fondyf_preA.php?id=$todo" target="_blank">Imprimible del Proyecto</a></p>
                <p align='left'>3. <a href="{$dna2url}frontcustom/284/ddjj_docu_fondyf_preA.php?id=$id" target="_blank">Listado de Documentaci&oacute;n a Presentar</a></p>
BLOCK;
        } else {
...
//        $renderData['text'] .=nl2br();
    ...
    function show_msgs($idcase) {
        ...
        $idwfs = array('fondyfpp', 'fondyfpde');
        foreach ($idwfs as $idwf) {
            ...
            $title = ($idwf == 'fondyfpp') ? "Pre Aprobados" : "Aprobados";
    ...
    function delegate_case() {
        ...
        $update = $this->fondyf_model->delegate_case_action($idwf, $idcase, $iduser_dest);
        return $update;
    }
    ...
    function widget_2doMe2($chunk = 1, $pagesize = 5) {
      ...
      echo $this->parser->parse('fondyf/widgets/2doMe2', $data, true, true);
    }
