...
//        exit;
        ...
        $rs = $this->db->get('container.proyectos_fonapyme')->result();

        foreach ($rs as $proj) {
            //$user=$this->user->get_user($proj->idu);
            $query = array('data.Proyectos_fonapyme.query.id' => $proj->id);
            $this->db->where($query);
            $this->db->select('id');
...
FROM `td_fonapyme` AS TF1
INNER JOIN `td_fonapyme` AS TF2 ON TF1.id = TF2.id
INNER JOIN `td_fonapyme` AS TF3 ON TF1.id = TF3.id
...
";
        ...
        foreach ($rs->result() as $row) {
            echo $row->id . '<br>';
            $case = array();
            $this->db->where(array('data.Proyectos_fonapyme.query.id' => (int) $row->id));
            $this->db->select('id');
//            $this->db->debug=true;
            $case = $this->db->get('case')->result();
            if (count($case)) {
    ...
    function fix_evaluador($case = null) {
        ...
        foreach ($cases as $case) {
            if ($case['id']) {
                ...
                $proy = $this->mongowrapper->db->selectcollection('container.proyectos_fonapyme')->findOne(array('id' => $case['id']), array('8668'));
                $ideval = $proy['8668'][0];
