...
class Inbox extends MX_Controller {
    ...
    function show_msgs_by_filter($filter = array()) {
        ...
        $customData['title'] = ($customData['qtty']!=0) ? "Notificaciones : Casos " . $filter['case'] : "No hay notificaciones para el caso " . $filter['case'];
    ...
    function get_users() {
        ...
        foreach ($allusers as $myuser) {
            /*         	var_dump($myuser);
              continue; */
            $name = "";
            if (!empty($myuser->name))
                $name.=$myuser->name;
            if (!empty($myuser->lastname))
