...
class Sgr extends MX_Controller {
    ...
    function __construct() {
        ...
        if ($this->session->userdata('iduser') == 253029915){     
    ...
    function render_config() {
        ...
        switch ($this->anexo) {
            case "07":
                $default_dashboard = 'dashboard_07';
...
//echo dirname(__FILE__); //$this->module_url;
            ...
            for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
                ...
                for ($j = 1; $j <= $data->sheets[0]['numCols']; $j++) {
                    if ($row_lenght > 1) {
                        ...
                        $valuesArr[] = array('fieldValue' => $fields, "row" => $i, "col" => $j, "count" => $count);
                    }
                }
            }
        ...
        if ($error==false) {
            ...
            /* INSERT UPDATE */
            for ($i = 2; $i <= $data->rowcount(); $i++) {
                if (!empty($data->sheets[0]['cells'][$i][1]) || $data->sheets[0]['cells'][$i][1] == 0) {
...
//$this->parser->parse('success2', $customData);
    ...
    function empty_xls_advice($anexo) {
        switch ($anexo) {
            ...
            case 126:
                $msg = "Sólo puede ser informada una fila.";
                break;
        }
    ...
    function print_anexo($parameter = null) {
        ...
        $anexo = (isset($this->session->userdata['anexo_code'])) ? $this->session->userdata['anexo_code'] : '06';
        $model = "model_" . $anexo;
        $this->load->model($model);
        ...
        $customData['user_print'] = strtoupper($lastname . ", " . $name);
    ...
    function set_period() {
        ...
        if ($period) {
            ...
            $set_start_month = strtotime(date('2010-12-30')); // strtotime(date('2013-12-30'));

            if ($this->idu == -342725103)
                $set_start_month = strtotime(date('2010-12-30'));

            /* #23828 */
            if ($this->sgr_id == 2519972722)
                $limit_month = strtotime('-1 month', strtotime(date('2014-11-01')));
    ...
    function set_no_movement() {
        ...
        if (!isset($get_period)) {
            ...
            if ($anexo == '141') {
                ...
                $rtn = (empty($check_141)) ? "error141" : "ok";
            }


            /* CHECK 202 "SIN MOVIMIENTO" VALIDATION */
            
            if ($anexo == '202' && $this->idu != 184254067) { //Support #25647

                $info_15 = $this->sgr_model->get_just_active("15", null, true);
                $rtn = (!empty($info_15)) ? "error202" : "ok";
            }
...
// testeo reemplazo appcache
        ...
        for ($i = date("Y"); $i > 2009; $i--) {
            ...
            foreach ($processed as $file) {
                ...
                if ($file_origen == "forms2") {
                    ...
                    $download = anchor('sgr/' . $asset . '/index/' . $anexo . '/' . $file_filename, ' <i class="fa fa-download" alt="Descargar"></i>', array('class' => 'btn btn-primary' . $disabled_link));
                    ...
                    $rectify = anchor($file['period'] . "/" . $anexo, '<i class="fa fa-undo" alt="Rectificar"></i> RECTIFICAR', array('class' => $rectifica_link_class . ' btn btn-warning'));
                    ...
                    $download = anchor('sgr/' . $asset . '/index/' . $anexo . '/' . $file_filename, ' <i class="fa fa-download" alt="Descargar"></i>', array('target' => '_blank', 'class' => 'btn btn-primary' . $disabled_link));
    ...
    function file_browser() {
        ...
        $customData = array();
        $customData['files_list'] = "<h1>FILE BROWSER</h1>";
        ...
        if (is_dir($absolute_path)) {
            ...
            /* parent folder ensure it exists and is the first in array */
            if ($path_in_url != '')
...
//$files_list .= anchor($prefix . $dir['name'], $dir['name']) . '<br>';
    ...
    function pre_general_validation($anexo, $error = null) {
        ...
        switch ($anexo) {
            ...
            case '202':
                ...
                $info_15 = $this->sgr_model->get_just_active("15");
                

                if (empty($info_15)) {
                    $error = true;
                    return "<strong>VG.2:</strong> <br>Desde que una SGR informó el 1º aporte al fondo de Riesgo, este anexo no puede estar en ningún período SIN MOVIMIENTOS, siempre tienen que informar los saldos. SALVO QUE, todos los aportes tengan saldo cero, el Anexo 14 tenga saldo cero y el Anexo 15 tenga saldo cero.
";
                }
                break;
        }
    ...
    function render($file, $customData) {
        ...
        $cpData['username'] = strtoupper($lastname . ", " . $name);
