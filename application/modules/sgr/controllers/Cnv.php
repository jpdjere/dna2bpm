 ...
 *
 */
class Cnv extends MX_Controller {
...
//$customData['layout']="layout.php"; 
    ...
    function Sgr_pick() {
        ...
        if (isset($this->session->userdata['sgr_impersonate']))
            $customData['menu_cnv'] = true;

        $this->render('cnv_template', $customData);
    }
    ...
    function get_sgrs() {
        $sgrArr = $this->cnv_model->get_sgrs();
        $sgrArr = (array) $sgrArr;
...
// PENDING LIST        
    ...
    function AnexosDB($target = '_self') {
        $module_url = base_url() . 'sgr/cnv/';
        $anexosArr = $this->cnv_model->get_anexos();
        $result = "";
    ...
    function oneAnexoDB() {
        $anexoValues = $this->cnv_model->get_anexo($this->anexo);
    ...
    function Anexo($filename = null) {
        ...
        $customData['sgr_nombre'] = $this->sgr_nombre;
        $customData['sgr_id'] = $this->sgr_id;
        $get_period = $this->cnv_model->get_processed($this->anexo, $this->sgr_id);
        $customData['js'] = array($this->module_url . "assets/jscript/dashboard.js" => 'Dashboard JS', $this->module_url . "assets/jscript/jquery-validate/jquery.validate.min_1.js" => 'Validate');
        $customData['css'] = array($this->module_url . "assets/css/dashboard.css" => 'Dashboard CSS');
...
//$this->parser->parse('success2', $customData);
    ...
    function print_anexo($parameter = null) {
        ...
        /* PERIOD INFO */
        $get_period_info = $this->cnv_model->get_period_filename($parameter);
    ...
    function print_ddjj($parameter = null) {
        ...
        foreach ($filenames_arr as $each) {
            $get_anexo = $this->cnv_model->get_period_data($each, $parameter, true);
...
// testeo reemplazo appcache
    ...
    function get_processed_17_tab() {
        ...
        for ($i = date("Y"); $i > 2009; $i--) {
            $processed = $this->cnv_model->get_ready($this->sgr_id, $i);
    ...
    function get_processed_17() {
        ...
        for ($i = date("Y"); $i > 2009; $i--) {
            ...
            for ($j = 12; $j > 0; $j--) {
                ...
                $processed = $this->cnv_model->get_ready_anexo($this->sgr_id, $new_query);
    ...
    function get_processed_tab($anexo) {
        ...
        for ($i = date("Y"); $i > 2009; $i--) {
            $processed = $this->cnv_model->get_processed($anexo, $this->sgr_id, $i);
            $processed = array($processed);
            foreach ($processed as $file) {
    ...
    function get_rectified_tab($anexo) {
        ...
        for ($i = date("Y"); $i > 2009; $i--) {
            $processed = $this->cnv_model->get_rectified($anexo, $this->sgr_id, $i);
            $processed = array($processed);
            foreach ($processed as $file) {
                if ($file)
        ...
        for ($i = date("Y"); $i > 2009; $i--) {
            ...
            foreach ($processed as $file) {
                ...
                if ($file['origen'] == "forms2") {
                    ...
                    $download = anchor('sgr/index/' . $asset . '/' . $anexo . '/' . $file['filename'], ' <i class="fa fa-download" alt="Descargar"></i>', array('class' => 'btn btn-primary' . $disabled_link));
                    ...
                    /* RECTIFY COUNT */
                    $count = $this->cnv_model->get_period_count($anexo, $file['period']);

                    $rectify_count_each = ($count > 0) ? "- " . $count . "º RECTIFICATIVA" : "";
                    $new_disabled_link = ($anexo == "09") ? ' disabled_link' : $disabled_link;
                    $download = anchor('sgr/index/' . $asset . '/' . $anexo . '/' . $file['filename'], ' <i class="fa fa-download" alt="Descargar"></i>', array('target' => '_blank', 'class' => 'btn btn-primary' . $disabled_link));
    ...
    function get_rectified($anexo) {
        ...
        for ($i = date("Y"); $i > 2009; $i--) {
            ...
            <div id="' . $i . '"><ul>';
            $rectified = $this->cnv_model->get_rectified($anexo, $this->sgr_id, $i);
    ...
    function get_pending($anexo) {
        ...
        $pending = $this->cnv_model->get_pending($anexo, $this->sgr_id);
