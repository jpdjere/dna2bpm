...
class Lib_06_error_legend {
    ...
    function return_legend($code, $row, $value) {
        ...
        switch ($code) {
            ...
            case "A.1":
                $result_error = '<strong>Columna A - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>Tipo de Operación: El campo no puede estar vacío y debe contener uno de los siguientes parámetros: "INCORPORACION, INCREMENTO TENENCIA ACCIONARIA", "DISMINUCION DE CAPITAL SOCIAL", "INTEGRACION PENDIENTE", "FUSION","FAE CORP","FAE GARANTAXI","FAE CUYO PYMES","FAE INCAA","FAE PMSA","FAE PRODAF","FAE CATAMARCA","FAE SANTA CRUZ","FAE SANTA FE","FAE SOCO RIL","FAE YAGUAR".';
                break;

            case "A.2":
                $result_error = '<strong>Columna A - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>Las opciones de FAE indicadas en el validador A.1 son sólo validad para las siguientes SGR:
                <br>GARANTIZAR S.G.R:
                FAE CORP
                FAE GARANTAXI
                FAE CUYO PYMES
            ...
            case "B.2.5":
                $result_error = '<strong>Columna B - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/> Si Columna A = FAE, solo se puede indicar B.';
                break;
            ...
            case "AF.2.5":
                $result_error = '<strong>Columna AF - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>Debe estar vacío.';
                break;
            ...
            case "AG.2":
                ...
                break;
            case "AG.2.5":
                $result_error = '<strong>Columna AG- Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>Debe estar vacío.';
                break;
            ...
            case "P.1": $result_error = '<strong>Columna P - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>P - WEB: OPCIONAL. De completarse, que tenga formato de dirección de página web (Ej. "www.industria.gob.ar").';
                break;
            case "Q.1": $result_error = '<strong>Columna Q - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>Q - Código de Actividad: El campo no puede estar vacío. Debe contener mínimo 5 y máximo 6 caracteres.';
                break;
            ...
            case "AK.3": $result_error = '<strong>Columna ' . $code . ' - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>EL SOCIO YA SE ENCUENTRA INCORPORADO A LA SGR, no puede incorporarlo nuevamente. ';
                break;
            case "Q.2": $result_error = '<strong>Columna Q - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>Q - Código de Actividad: Para los Socios (Partícipes o Protectores) incorporados hasta Octubre de 2013 inclusive, debe validar que se corresponda con alguno de los códigos del FORMULARIO AFIP F. Nº 150/1999, cuyo listado figura en el Archivo Excel adjunto al instructivo.';
                break;
            ...
            case "AB.1": $result_error = '<strong>Columna AB - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>Cantidad de Empleados: El campo no puede estar vacío y debe contener caracteres numéricos mayores a Cero.';
                break;
            case "Q.2.A": $result_error = '<strong>Columna Q - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>Para los Socios Partícipes (A) incorporados hasta Octubre de 2013 inclusive, debe validar que se corresponda únicamente con alguno de los códigos del archivo 06 - CODIGOS DE ACTIVIDAD VIEJOS - F. 150-1999 - socios A.';
                break;
             case "Q.2.B": $result_error = '<strong>Columna Q - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>Para los Socios Partícipes (B) incorporados hasta Octubre de 2013 inclusive, debe validar que se corresponda únicamente con alguno de los códigos del archivo 06 - CODIGOS DE ACTIVIDAD VIEJOS - F. 150-1999 - socios B.';
                break;
            case "Q.3": $result_error = '<strong>Columna Q - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>Q - Código de Actividad: Para los Socios (Partícipes o Protectores) incorporados a partir de Noviembre de 2013 inclusive, debe validar que se corresponda con alguno de los códigos del FORMULARIO AFIP F. Nº 883/2013, cuyo listado figura en el Archivo Excel adjunto al instructivo.';
                break;
            case "Q.3.A": $result_error = '<strong>Columna Q - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>Para los Socios Partícipes (A) incorporados a partir de Noviembre de 2013 inclusive, debe validar que se corresponda únicamente con alguno de los códigos del archivo 06 - CODIGOS DE ACTIVIDAD NUEVOS - F 883-2013 - socios A.<br/>.';
                break;
            ...
            case "C.2": $result_error = '<strong>Columna C - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>Debe verificar que el CUIT que se esté incorporando no esté incorporado previamente, para la cual deber verificar que no tenga saldos de Capital positivos (Tanto Suscriptos como Integrados), en la SGR en que se está incorporando.';
                break;
            case "C.2.5": $result_error = '<strong>Columna C - Fila Nro.' . $row . ' - Código Validación ' . $code . '</strong><br/>Debe verificar que el CUIT que se esté incorporando no esté incorporado previamente, con el código indicado en la columna A, en la SGR en que se está incorporando.';
                break;
