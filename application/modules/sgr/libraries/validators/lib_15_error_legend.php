...
class Lib_15_error_legend {
    ...
    function return_legend($code, $row, $value) {
        ...
        switch ($code) {
            case "A.1":
                $result_error = '<strong>Columna A - Fila Nro.'. $row .' - Código Validación '.$code.'</strong><br/>Debe estar compuesta por alguno de los parámetros establecidos en la Columna A de Anexo adjunto (OPCIONES DE INVERSIÓN) a tales efectos.';
                break;
            case "B.1":
            ...
            case "F.1":
                ...
                break;
            case "G.1":
                $result_error = '<strong>Columna G - Fila Nro.'. $row .' - Código Validación '.$code.'</strong><br/>Debe tener 11 caracteres numéricos sin guiones. Debe validar que se corresponda con alguno de los CUIT detallados en el Anexo adjunto, donde se listan las ENTIDADES DEPOSITARIAS habilitadas a tales efectos.';
                break;
             case "H.1":
