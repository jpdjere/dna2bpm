...
class Management_model extends CI_Model {
    ...
    function get_pending($anexo, $sgr_id,$year = null) {
        $rtn = array();
        $regex = new MongoRegex('/' . $year . '/');
        $container = 'container.sgr_periodos';
    ...
    function get_active_tmp($anexo, $exclude_this = false) {
        ...
        $result = $this->mongowrapper->sgr_tmp->$period->find($query);
...
//             echo $myid."/";
//             $query=array("id"=>$myid);
//             $rs = $this->mongowrapper->sgr->$container->update($query, array('$set'=>$data), $options);
//             //if($i==1)break;
//         }
//             
//
