...
function run_StartNoneEvent($shape, $wf, $CI) {
    ...
    //----Set status 4 Case
    $CI->bpm->update_case($wf->idwf, $wf->idcase, array('status' => 'open'));
}
    ...
    if ($parent) {
        switch ($parent->stencil->id) {
            ...
            default:
                ...
                $active_tokens = $CI->bpm->get_pending($wf->idwf, $wf->idcase, array('user', 'waiting', 'pending'), array());
                if ($active_tokens->count() == 0) {
                    $CI->bpm->update_case($wf->idwf, $wf->idcase, array(
                        'status' => 'closed',
                ...
                $mycase = $CI->bpm->get_case($wf->idcase, $wf->idwf);
...
function run_EndErrorEvent($shape, $wf, $CI) {
    ...
    run_IntermediateEventThrowing($shape, $wf, $CI);
    //---Update case Status
    $CI->bpm->update_case($wf->idwf, $wf->idcase, array('status' => 'canceled'));
    //---then move next
}
