 ...
 * incluye datos desde los containers respectivos contemplando así el historico
 *
 * @author Juan Ignacio Borda <juanignacioborda@gmail.com>
 * @date    Jan 28, 2015
 */
class Api11_all extends MX_Controller {
    ...
    function __construct() {
        ...
        //---default queries
        $this->container='container.proyectos_pacc';
        $this->postTitle=' H';
        $this->queryTotal=array('6225'=>array('$gte'=>20),'status'=>'activa');
        /* LOAD MODEL */
    ...
    function Presentados($filter = null, $mode = 'json') {
        $title = 'Presentados';
        $total = $this->db->where($this->queryTotal)->count_all_results($this->container);
        $query['6225.0'] = array('$gte'=>20);
        $query['status'] = 'activa';

        $cant = $this->db->where($query)->count_all_results($this->container);
        $label = "$cant";
    ...
    function Presentados_pitch($filter = null, $mode = 'json') {
        $title = 'Presentados Pitch';
         $total = $this->db->where($this->queryTotalPitchs)->count_all_results($this->container);
        $query['8926.0'] = array('$gte'=>7);
        $query['status'] = 'activa';
    ...
    function Primer_pago($filter = null, $mode = 'json') {
       ...
       $total = $this->db->where($this->queryTotal)->count_all_results($this->container);
        $query= array(
            "distinct" => "container.actividades_pacc_11", 
            "key" => "parent.id",
            "query" =>array(6895=>array('$exists'=>true),'status'=>'activa')
            );
        $rs=$this->db->command($query);
        $cant = count($rs['values']);
        $label = "$cant";
        $porc = @ intval(($cant / $total) * 100);
        $data = array(
            'value' => $porc,
     ...
     *
     * Metas para Empresas - Finalizados
     * META = Universo PDEs (total de casos del flujo paccPagoSD)
    ...
    function Finalizados($filter = null, $mode = 'json') {
        $title = 'Finalizados';
        $total = $this->db->where($this->queryTotal)->count_all_results($this->container);
        $query['6225.0'] = array('$in'=>array(480,500));
        $query['status'] = 'activa';
        $cant = $this->db->where($query)->count_all_results($this->container);
        $label="$cant";
        $porc=  intval(($cant/$total)*100);
